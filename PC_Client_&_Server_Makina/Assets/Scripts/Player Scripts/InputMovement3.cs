...
public class InputMovement3 : MonoBehaviour
{
    ...
    [SerializeField, HideInInspector, Tooltip("The camera transform (not the parent)")] private Transform m_cameraTr = null;
    [SerializeField, HideInInspector, Tooltip("The camera parent transform")] private Transform m_cameraParentTr = null;
    ...
    [SerializeField, HideInInspector, Tooltip("Will the player jump using the ground normal when grounded")]private bool m_jumpUsingGroundNormal = false;
    ...
    Vector3 m_slopeDisplacement = Vector3.down;
    
    /// <summary/> The current pitch of the player's first person camera
    private float m_cameraPicth;
    ...
    {
        UpdateMouseLook();
        GroundTouchingState groundTouchingState;
        Vector3 groundNormal;
        Vector3 displacement = new Vector3();

        UpdateGroundDetection(out groundTouchingState, out groundNormal);
    ...
    private void UpdateGroundDetection(out GroundTouchingState p_groundTouchingState, out Vector3 p_groundNormal)
    {
        ...
        bool groundDetected = Physics.SphereCast(origin, rad, Vector3.down, out groundHit, m_groundCheckRange);
    ...
    /// <summary/> This caroutine will extends the amount of time the player will be able to jump
    ...
    /// <param name="p_groundTouchingState"> Wether the player is grounded or whatever  </param>
    ...
    private void UpdateJump(GroundTouchingState p_groundTouchingState, Vector3 p_groundNormal)
    {
        ...
        if(p_groundTouchingState != GroundTouchingState.grounded && p_groundTouchingState != GroundTouchingState.onSlope)
        {
            ...
            bool bumpedHead = Physics.SphereCast(origin, rad, Vector3.up, out RaycastHit hit, m_controller.skinWidth + rad + Mathf.Epsilon + m_controller.minMoveDistance);
        ...
        // Jumping with the On slope condition if the player is onslope
    ...
    {
        // Checking wether or not we should apply the gravity
    ...
    private void UpdatePlayerInput(ref Vector3 p_displacement, Vector3 p_groundNormal)
    {
        ...
        //Switch case for the different acceleration curves goes brrr
    ...
    /// <summary/> resets the curve timer by reseyting the timer and getting a new initial speed
    ...
    void UpdateMouseLook()
    {
        ...
        m_cameraPicth -= mouseDelta.y * m_mouseSensitivity;
