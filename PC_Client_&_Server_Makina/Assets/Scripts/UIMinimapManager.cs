...
public class UIMinimapManager : MonoBehaviour {
    ...
    [SerializeField] [Range(1f, 50f)] [Tooltip("How many times the map should be zoomed in\nWarning, if this value is changed in play mode, it will NOT be taken into account")] private float f_mapZoom = 4f;
    ...
    private void Start() {
        m_mapZoom = f_mapZoom;
        m_mapElement.localScale = new Vector3(m_mapZoom, m_mapZoom, m_mapZoom);
    ...
    void Update() {
        ...
        Vector2 playerPositionRatio = new Vector2(playerPosition.x / (m_mapRealSize/2f), playerPosition.z / (m_mapRealSize/2f)); //This line gives a vector2 with both x and y being between -1 and 1 and corresponds to how far the player is from the center (0 is the center, 1 is the farthest on one side and -1 is the farthest on the opposite side)

        m_mapElement.anchorMin = new Vector2(playerPositionRatio.x - (m_mapZoom / 2f), playerPositionRatio.y - (m_mapZoom / 2f));
        m_mapElement.anchorMax = new Vector2(playerPositionRatio.x + (m_mapZoom / 2f), playerPositionRatio.y + (m_mapZoom / 2f));
