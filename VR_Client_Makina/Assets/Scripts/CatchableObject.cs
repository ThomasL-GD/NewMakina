...
public abstract class CatchableObject : MonoBehaviour {
    ...
    private bool m_hasBeenCaughtInLifetime = false;
    public bool m_isGrabable = true;
    public List<Transform> m_originalParent = new List<Transform>();

    protected bool m_isPuttableOnlyOnce = false; // If true, once this object is let go somewhere, it can NOT be picked up again

    public delegate void DestroyCatchableDelegator(CatchableObject p_catchableObject);
    /// <summary>Is called when this object will be destroyed</summary>
    public DestroyCatchableDelegator OnDestroyCatchable;
    ...
    public void DestroyMaSoul() {
        OnDestroyCatchable?.Invoke(this);
