...
public class LocalLaser : MonoBehaviour {
    ...
    void Update() {
        ...
        if (m_elapsedTime > m_laserLoadingTime) {
            ...
            StartCoroutine(ShotDownLaser(hit, hasRaycastHit));
        }
    ...
    IEnumerator ShotDownLaser(RaycastHit p_hit, bool p_hitButton) {
        yield return new WaitForSeconds(m_laserDuration);
        m_line.enabled = false;
                
        if (p_hitButton && MyNetworkManager.singleton.m_canSend) {
